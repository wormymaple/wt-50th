...
func new_scene(passed) -> void:
	if !passed: #This makes sure that the scene will not break if a player somehow fails and succeeds at a game
	...
	TimeBar.set_max(NewTimerMax) #This updates the max value of the timer which is necessary because the bar operates on percentage of time left
...
func _fail():
	...
	game = null
	GameTimer.stop()
	$CanvasLayer/PassFail.show_fail()
...
func _on_GameHolder_child_entered_tree(_node):
